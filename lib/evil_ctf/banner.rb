  ...
  module Banner
    ...
    module Color
      def self.header(text, color = :cyan)
        ...
        when :blue then text.blue
        when :magenta then text.magenta
        when :white then text.white
        when :cyan then text.cyan
        else; text
        end
      end
    ...
    def self.show_minimal_banner(shell, options, no_color: false)
      ...
      begin
        ...
        integrity = probe_out.lines.find do |ln|
          ln.start_with?('INTEGRITY|||')
      ...
      # Defender Status
      begin
        defender = EvilCTF::Execution.run(shell, '(Get-MpComputerStatus).RealTimeProtectionEnabled',
                                          timeout: 8).output.to_s.strip
    ...
    def self.show_expanded_banner(shell, options, no_color: false)
      ...
      begin
        ...
        if services_output.lines.count > 3
          ...
          services_output.lines[3..6].each do |line|
            ...
            puts "      #{cleaned_line}" if cleaned_line.length.positive?
          end
      ...
      begin
        ...
        result.output.each_line do |line|
          ...
          next if unique_paths.include?(path)

          unique_paths.add(path)
          puts "  [!] Found: #{path}".red
          puts "      #{value.strip}" if value && value.strip.length.positive?
        end
    ...
    def self.display_lateral_movement_opportunities(shell)
      ...
      puts '  [→] Admin Shares: Available (ADMIN$, C$)'.green if shares.include?('ADMIN$') || shares.include?('C$')
      ...
      puts '  [→] SMB Signing: Disabled (SMB relay possible)'.red if smb_signing == 'False'
      ps_remoting = shell.run('Get-PSSessionConfiguration 2>$null').output
      puts '  [→] PS Remoting: Enabled'.green if ps_remoting.include?('Microsoft.PowerShell')
      winrm = shell.run('Get-Service WinRM 2>$null').output
      puts '  [→] WinRM: Running (lateral movement possible)'.green if winrm.include?('Running')
