...
- [x] 🟡 **Consolidate bypass scripts** — ✅ 2026-08-18: all four constants plus the applier moved to `lib/evil_ctf/bypass.rb` as `EvilCTF::Bypass` (`Bypass.apply(shell, amsi:, etw:, verify:, verbose:)`); the four heredocs are byte-identical after the move (verified by value comparison). Updated references: `macro_engine` (17 macro-step sites), `command_dispatcher` (bypass-4msi/bypass-etw commands), `runtime_setup` + `tui` (applier calls); specs split into `spec/bypass_spec.rb` (constants + apply, moved intact) leaving `tools_bypass_spec.rb` for the rest of Tools. The `Tools` module dropped ~140 lines — the `Metrics/ModuleLength` cop exemption is gone (the scoped `MethodLength` exemption on the tool-catalog builder remains, legitimately)
...
- [x] 🔴 **Add connection pooling for multi-host mode** — ✅ 2026-08-18: new `EvilCTF::ConnectionPool` (`lib/evil_ctf/connection_pool.rb`): process-wide, keyed by endpoint + user + auth-mode (password/hash/kerberos) + ssl + transport, LRU-capped at 8, mutex-protected. `acquire` (build-or-reuse via `Connection.build_full`), `register` (hand an externally validated connection in), `evict` (remove + close), `detach`, `close_all`. Wiring: `Session.test_connection` now hands its validated connection to the pool instead of closing it, so `Bootstrap.build_connection` (now a pool acquire) reuses it — a `--verify` single-host session and both TUI extra-session flows pay for one WinRM handshake instead of two; the TUI extra-session `build_full` calls became pool acquires (key normalization mirrors `build_full`'s ip:+port: → endpoint resolution); the reconnect-retry path in `run_session` now evicts the dead connection (and stops the leaked heartbeat + closes the leaked shell — retry re-runs the begin block, the ensure only runs on final exit) so each retry gets a fresh connection; session end evicts+ closes the connection (previously leaked — `run_session` never closed its conn); CLI closes the pool at both exit points. 14 specs (`spec/connection_pool_spec.rb`) cover caching, keying, ip:/endpoint: equivalence, LRU cap, builder failure, concurrent acquire (16 threads → 1 build), register handoff, evict/detach/close_all, and the test_connection → Bootstrap.build_connection handoff (success reuses, failure closes)
...
- [x] 🔴 **Handle upload resume after partial failure** — ✅ 2026-08-18: chunked uploads now genuinely resume. The old code generated a *random* `.part_<token>` name per call, recreated it empty in init, then probed that same fresh file for an offset — resume was dead code, and `ensure` deleted the partial on every failure. Now: deterministic `<final>.part` + a `<final>.part.meta` sidecar recording the SHA-256 of what the partial was built from; `resolve_resume_offset` probes both in one round-trip and resumes only when the sidecar proves the partial is a prefix of the current local file (transformed bytes for XOR/encrypt uploads), aligning to chunk boundaries and trimming a ragged trailing chunk via `SetLength`. A complete-but-unmoved partial skips chunking and goes straight to move+verify. Failed attempts KEEP the partial + sidecar (success removes the sidecar); `resume: false` forces a fresh upload (wired to the new `--no-resume` CLI flag, `--fresh`, and the stager/dispatcher/menu call sites). Also fixed a latent bug found en route: `verify: true` compared the remote hash against the *raw* local hash, which failed every XOR/encrypted upload — verification now uses the hash of the transformed bytes. 10 new specs (`spec/uploader_resume_spec.rb`) emulate the remote side in memory: fresh / resume / trim / mismatched meta / missing meta / force-fresh / complete partial / failure keeps partial / XOR verify / XOR resume
...
- [x] 🔴 **No integration tests** — ✅ 2026-08-18: `spec/integration/` harness landed — live WinRM connection (build_full, ConnectionValidator, `Execution.run` command execution, OS product data) and an upload → `Test-Path` → download round trip with byte-for-byte verification, all through product code paths. The `integration-tests` CI job always runs and the specs self-skip unless `AWINRM_INTEGRATION=1` (repo variable) + `WINRM_TEST_HOST/USER/PASSWORD|HASH` secrets are set; deliberately no job-level `if` (the env context is unavailable to job `if` expressions, and the April bug was a malformed job `if` aborting the whole workflow dispatch). CI verified green (job = 6 skips, 36s). First live run against a Windows test VM still pending — set the repo vars/secrets to enable it
...
- [x] 🟢 **Add CI Ruby matrix** — ✅ 2026-08-18: `unit-tests` now runs a 3.2/3.3/4.0 matrix (fail-fast: false) with frozen `bundler-cache` in every job, matching the "Ruby 3.0+ / 4.0-ready" story — verified green on all three (run 32170170467). Root cause of the old 3.2 failure: `parallel 2.1.0` (transitive dep of rubocop) requires Ruby ≥ 3.3; fixed by pinning `parallel ~> 1.28` in the Gemfile so one lockfile installs frozen on every matrix ruby (all 60 locked gems audited for 3.2 compatibility). No more per-ruby lockfile re-resolution
...
- [x] 🟡 **Add command reference guide** — ✅ 2026-08-18: README documents all built-in commands (incl. `tools`, `tool <name>`, `download_missing`, `load_ps1`, `invoke-binary`, `get-unquotedservices`), 15 macros with staged dependencies, and shell aliases; built-in `help` output also gained `tool <name> / tool all` (was omitted)
